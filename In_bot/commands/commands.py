"""
commands.py — Hub, Login, Settings, Health, Homeworks, Leaderboard
Uses async LNApiClient from automation.api_direct (the working login path).
"""
...
async def do_login(username: str, password: str) -> Tuple[bool, str]:
    """Login using LNApiClient (async, proven working path)."""
    try:
        client = LNApiClient()
        result = await client.login(username, password)
        if client.token:
            return True, client.token
        return False, f"No token: isLoggedIn={result.get('isLoggedIn')}, newToken='{result.get('newToken', '')[:20]}'"
    except Exception as e:
        return False, f"{type(e).__name__}: {str(e)[:200]}"

async def check_health(username: str, password: str) -> dict:
    """Login and check account health. Returns banned dict."""
    ok, token_or_err = await do_login(username, password)
    if not ok:
        return {"banned": True, "status": "login_failed", "error_message": token_or_err, "unban_in": None}
    try:
        client = LNApiClient()
        client.token = token_or_err
        stats = await client.call_lnut("stats/get", {"token": token_or_err})
        if stats.get("error"):
            return {"banned": True, "status": "error", "error_message": str(stats.get("body", "")), "unban_in": None}
        # Banned accounts return certain indicators
        msg = ""
        if isinstance(stats, dict):
            msg = str(stats.get("msg", stats.get("message", "")))
        is_banned = bool(msg) and any(kw in msg.lower() for kw in ["ban", "suspend", "deni", "block"])
        return {
            "banned": is_banned,
        ...
        return {"banned": True, "status": "error", "error_message": str(e), "unban_in": None}
...
class HubView(ui.View):
    ...
    async def health(self, interaction: Interaction, btn: ui.Button):
        ...
        if health.get("banned"):
            e = discord.Embed(title="\u274C Account Issue", description=f"Status: **{health['status']}**\n{health['error_message']}", color=RED)
        else:
...
class Commands(commands.Cog):
    ...
    async def homeworks(self, interaction: Interaction):
        ...
        try:
            client = LNApiClient()
            client.token = token
            result = await client.call_lnut("homeworkController/findHomeworkForStudent", {"token": token})
        except Exception as e:
    ...
    async def account_health(self, interaction: Interaction):
        ...
        health = await check_health(uname, pwd)
        if health.get("banned"):
            e = discord.Embed(title="\u274C Account Issue", description=f"Status: **{health['status']}**\n{health['error_message']}", colour=RED)
        else:
