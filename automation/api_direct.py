...
class LNApiClient:
    ...
    async def submit_score(
        ...
        Payload mirrors JS send_answers() exactly:
          - score = vocabs.length * xp_per_task (default 200)
        ...
        incorrect_vocabs = [task_data[i].get("uid", "") for i in sorted(incorrect_indices) if i < len(task_data)]

        # JS: score = vocabs.length * xp_per_task  (uses TOTAL count, not just correct)
        score = len(task_data) * xp_per_task
