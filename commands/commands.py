...
from utils import action_logger
...
# Per-Question Timing
        ...
        embed.add_field(
            name="⭐ XP Per Task",
            value=f"`{s['xp_per_task']}` XP per vocab item",
            inline=True,
        )
    ...
    @ui.button(label="⭐ XP Per Task", style=ButtonStyle.success, row=2)
    async def xp_per_task_btn(self, interaction: Interaction, _: ui.Button):
        s = config.get_guild_settings(interaction.guild_id)
        await interaction.response.send_modal(
            SettingModal(
                "xp_per_task", "XP per task item (10-5000)",
                s["xp_per_task"], int, lambda v: 10 <= v <= 5000, self,
            )
        )
    # ── Toggle buttons ────────────────────────────────────────────────────
...
async def _execute_jobs(
    ...
    if not client:
        try:
            await followup.send("❌ Not logged in. Use `/login` first, or read `/tutorial` to learn how to use the bot.", ephemeral=True)
        except Exception:
            pass
        return
        ...
        async with sem:
            ...
            while True:
                ...
                try:
                    ...
                    xp_val = settings.get("xp_per_task", 200)
                    result = await client.submit_score(t_obj, data, hw, xp_per_task=xp_val)
...
class BotCommands(commands.Cog):
    ...
    async def homework(self, interaction: Interaction):
        ...
        if not client:
            await interaction.followup.send("❌ Not logged in. Use `/login` first, or read `/tutorial` to learn how to use the bot.", ephemeral=True)
            return
    ...
    # CHANGELOGS
    # =========================================================
    @app_commands.command(name="changelogs", description="Show recent bot updates and changes")
    async def changelogs_cmd(self, interaction: Interaction):
        """Display CHANGELOG.md in a paginated embed."""
        await interaction.response.defer(ephemeral=True)
        changelog_paths = [
            "CHANGELOG.md",
            os.path.join(os.path.dirname(__file__), "..", "CHANGELOG.md"),
        ]
        cl_file = next((p for p in changelog_paths if os.path.exists(p)), None)
        if not cl_file:
        ...
        try:
            ...
            await interaction.followup.send(f"Error reading changelog: {e}", ephemeral=True)
            return
        pages = []
        current_section = ""
        current_lines = []
        ...
        if current_lines:
            ...
            pages.append((current_section, body))
        if not pages:
            await interaction.followup.send("Changelog is empty.", ephemeral=True)
            return
        embed = discord.Embed(
            title=pages[0][0],
            description=pages[0][1][:4000],
        ...
        embed.set_footer(text="Page 1/" + str(len(pages)) + " | /commands-help for command usage")
        await interaction.followup.send(embed=embed, ephemeral=True)
    ...
    # COMMANDS HELP
    # =========================================================
    @app_commands.command(name="commands-help", description="Show how to use a command")
    @app_commands.describe(cmd="The command to learn about")
    async def commands_help_cmd(self, interaction: Interaction, cmd: str):
        """Display usage guide for a specific command."""
        await interaction.response.defer(ephemeral=True)

        is_owner = interaction.user.id == OWNER_ID

        all_commands = {
            "login": {"desc": "Log in to LanguageNut and securely store your credentials", "usage": "/login <username> <password>", "admin": False, "note": "Your credentials are encrypted before storage."},
            "logout": {"desc": "Remove stored LanguageNut credentials and sign out", "usage": "/logout", "admin": False, "note": "Deletes your saved login from this server."},
            "homework": {"desc": "List all your homework assignments with progress", "usage": "/homework", "admin": False, "note": "Shows each assignment with task completion status."},
            "quick-do": {"desc": "Quickly complete a task by homework:index number", "usage": "/quick-do <task>", "admin": False, "note": "Example: /quick-do 3:2 does homework #3, task index 2."},
            "do": {"desc": "Complete homework tasks using an interactive menu", "usage": "/do", "admin": False, "note": "Pick from assignments and choose which tasks to run."},
            "xpgrind": {"desc": "Grind XP by completing tasks in a language", "usage": "/xpgrind", "admin": False, "note": "Select a language and it runs all unfinished tasks for XP."},
            "settings": {"desc": "Open the bot settings panel", "usage": "/settings", "admin": False, "note": "Adjust accuracy, timing, XP per task, and stealth mode."},
            "tutorial": {"desc": "Read the bot tutorial / user guide", "usage": "/tutorial", "admin": False, "note": "Step-by-step guide on how to use the bot."},
            "status": {"desc": "Show bot status and info", "usage": "/status", "admin": False, "note": "Shows server count, latency, Python version, login status."},
            "leaderboard": {"desc": "View leaderboard rankings and XP needed for a target rank", "usage": "/leaderboard <class|school|world> [target_rank]", "admin": False, "note": "Optional target rank shows XP gap to reach it."},
            "changelogs": {"desc": "Show bot updates and change history", "usage": "/changelogs", "admin": False, "note": "Plain-English summary of what has changed recently."},
            "commands-help": {"desc": "Get help with any command", "usage": "/commands-help <command>", "admin": False, "note": "Pick a command to see its usage guide."},
            "restart": {"desc": "Restart the bot (owner only)", "usage": "/restart", "admin": True, "note": "Bot shuts down and starts again."},
            "shutdown": {"desc": "Stop the bot (owner only)", "usage": "/shutdown", "admin": True, "note": "Bot goes offline."},
            "sync": {"desc": "Sync slash commands (owner only)", "usage": "/sync", "admin": True, "note": "Registers all commands with Discord."},
            "update": {"desc": "Git pull + restart (owner only)", "usage": "/update", "admin": True, "note": "Pulls latest code from GitHub then restarts."},
            "clear": {"desc": "Delete recent messages (owner only)", "usage": "/clear <amount>", "admin": True, "note": "Deletes up to 100 messages from current channel."},
            "logs": {"desc": "Show last 20 log lines (owner only)", "usage": "/logs", "admin": True, "note": "Useful for debugging issues."},
            "reload": {"desc": "Reload a cog (owner only)", "usage": "/reload <cog>", "admin": True, "note": "Reloads a command module without full restart."},
            "eval": {"desc": "Execute Python code (owner only)", "usage": "/eval <code>", "admin": True, "note": "Runs arbitrary Python for debugging."},
            "online": {"desc": "Announce bot is online (owner only)", "usage": "/online", "admin": True, "note": "Sends @everyone message."},
            "offline": {"desc": "Announce bot is offline (owner only)", "usage": "/offline", "admin": True, "note": "Sends @everyone message."},
            "say": {"desc": "Make the bot say something (owner only)", "usage": "/say <message>", "admin": True, "note": "Bot sends your message to the channel."},
            "embed": {"desc": "Send an embedded message (owner only)", "usage": "/embed <title> <description> [color]", "admin": True, "note": "Formats a nice embed card."},
            "userinfo": {"desc": "Show info about a user (owner only)", "usage": "/userinfo <user>", "admin": True, "note": "Shows ID, join date, roles."},
            "serverinfo": {"desc": "Show info about this server (owner only)", "usage": "/serverinfo", "admin": True, "note": "Shows member count, channels, owner."},
            "dm": {"desc": "DM a user (owner only)", "usage": "/dm <user> <message>", "admin": True, "note": "Sends a private message."},
            "nickname": {"desc": "Change a user's nickname (owner only)", "usage": "/nickname <user> [nickname]", "admin": True, "note": "Leave nickname blank to reset."},
            "purge": {"desc": "Bulk delete messages (owner only)", "usage": "/purge [amount]", "admin": True, "note": "Deletes up to 500 messages."},
            "lockdown": {"desc": "Lock or unlock a channel (owner only)", "usage": "/lockdown <lock|unlock>", "admin": True, "note": "Restricts @everyone from sending messages."},
        }

        cmd_lower = cmd.lower().lstrip("/").replace(" ", "-").replace("_", "-")
        info = all_commands.get(cmd_lower)

        if not info:
            matches = [k for k in all_commands if k.startswith(cmd_lower[:3])]
            if matches:
                await interaction.followup.send(
                ...
                await interaction.followup.send("Unknown command: `" + cmd + "`. Try /commands-help with a valid command name.", ephemeral=True)
            return

        if info["admin"] and not is_owner:
            await interaction.followup.send("That command is owner-only and not available to you.", ephemeral=True)
            return
        ...
        if info["admin"]:
            ...
            embed.set_footer(text="Available to all users")
